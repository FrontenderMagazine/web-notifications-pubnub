...
W3C не оговаривает, как уведомления должны выглядеть, так что Opera решила 
использовать нативные уведомления, чтобы браузер казался полностью интегрированным 
в операционную систему.

![Скриншот][Пример уведомления]

Уведомления поддерживаются в Opera начиная с 25-й версии. Статья 
«[Уведомления в 25 версии Opera для разработчиков][3]» содержит базовые
...
позволяет разработчикам в реальном времени создавать, масштабировать и управлять 
функциональностью приложений и устройств в интернете вещей. (Признаюсь: 
я работаю в PubNub)
...
Для этой статьи я разработала альтернативу приложения «Yo», которое позволяет
...
так как ещё [не все браузеры этот API поддерживают][7].
...
в первый раз, пользователь получает запрос на использование API страницей. 
...
открыв opera://settings, выбрав раздел Сайты в левой колонке и прокрутив
...
чтобы начать работу со страницей.

    <input type="text" placeholder="Наберите имя, чтобы начать работу" autofocus>
    
Когда пользователь набирает имя и нажимает кнопку ввода (мы будем отлавливать 
...
[создать бесплатную учетную запись][10]. Кнопки опубликовать и подписаться 
находятся в [панели управления разработчика][11].
...
или офлайн. Это полезно для того, чтобы поддерживать список пользователей,
