# Работа с уведомлениями и использование PubNub для создания потоков 
...
показывается даже если страница неактивна — однако надо иметь в виду, что она 
все же должна быть открыта в одной из фоновых вкладок, что бы создать уведомление.
W3C не оговаривает, как уведомления должны выглядеть, так что Опера решила 
использовать нативные уведомления, что бы браузер казался полностью интегрированным 
...
*Обобщонная парадигма обмена сообщениями на основе подписки и их публикации*
...
разрешения использования Web Notification API страницей, что бы пользователь мог 
...
что бы начать работу со страничкой.

    <input type="text" placeholder="Наберите имя, что бы начать работу" autofocus>
    
Когда пользователь вводити имя и нажимает кнопку ввода (мы будем отлавливать 
...
что бы зарегистрировать пользователя.
    ...
    document.querySelector('[type="text"]').addEventListener('keyup', function(event) {
      ...
      // Соеденяемся с PubNub и регистрируем пользователя …
    });
...
или оффлайн. Это полезно для того, что бы поддерживать список пользователей, подписанных на канал, в актуальном состоянии.
...
Что бы упростить статью и сфокусироваться на важном, я упускаю детали процесса 
...
только аттрибуты `from` и `to`, но может быть любым.
...
туда поместить. Так что можете изменить скрипт так, что бы вместо отправляемого 
...
Что бы максимально упростить пример, вместо того, что бы получать приватное
...
Что бы получать сообщения в реальном времени просто вызовите метод `subscribe()` 
PubNub API. Когда сообщение придет выбранному вами пользоватенлю, он увидит 
уведомление.
...
Тег, присвоенный каждому экземпляру уведомления, используется что бы контролировать количество уведомлений, которые будут покаывать. Например, даже если приложение 
...
[Парадигма обмена сообщениями на основе подписки и их публикации]: img/pubnub-publish-subscribe-paradigms.png "Парадигма обмена сообщениями на основе подписки и их публикации"
...
[Отправка сообщения всем подписчикам]: img/publish-subscribe-oi.png "Отправка сообщения всем подписчикам"
